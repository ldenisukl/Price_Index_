...
type PriceEntryWithRelations = Prisma.PriceEntryGetPayload<{
  include: {
...
export async function GET(request: Request) {
  ...
  for (const entry of allEntries) {
    const key = `${entry.priceItemId}|${entry.regionId}|${entry.providerName ?? 'null'}|${entry.priceType}`;
  ...
  const data = priceEntries.map((entry: PriceEntryWithRelations) => {
    ...
    const subtitle = `${providerLabel}${capitalize(entry.priceItem.category.type)} · ${entry.region?.name ?? 'Unknown'}`;
    const historyKey = `${entry.priceItemId}|${entry.regionId}|${entry.providerName ?? 'null'}|${entry.priceType}`;
